...
public class AdbcommandActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle _savedInstanceState) {
		...
		setContentView(R.layout.adbcommand);
		initialize(_savedInstanceState);
		initializeLogic();
		
		ADB adb = ADB.getInstance(this);

        adb.started.observe(this, isStarted -> {
    
        if (isStarted) {
        textview1.append(parseAnsi("[HM] ADB Ready\n", true));
    }
});
	}
	...
	private void finishExecution() {
        ...
        try {
            ...
            if (!Boolean.TRUE.equals(adb.started.getValue())) {
                textview1.append(parseAnsi("[HM] ERROR: ADB tidak terhubung!\n", true));
                return;
            }
            ...
            new Thread(() -> {
                try {
                    ...
                    runOnUiThread(() -> textview1.append(parseAnsi("[HM] Shell PID : " + pid + "\n", true)));
                    
                    String tokenSelesai = "KODE_SELESAI_" + System.currentTimeMillis();

                    String fullCommand = command + " \n echo '" + tokenSelesai + "'";
                    
                    adb.sendToShellProcess(fullCommand);
                    
                    java.io.File bufferFile = adb.outputBufferFile;
                    long lastReadPosition = bufferFile.length(); 
                    ...
                    while (isRunning) {
                        ...
                        if (bufferFile.exists()) {
                            ...
                            if (currentLength > lastReadPosition) {
                                java.io.RandomAccessFile raf = new java.io.RandomAccessFile(bufferFile, "r");
                                ...
                                String newLog = new String(newBytes);
                                lastReadPosition = currentLength;                   
                                runOnUiThread(() -> {
                                    if (!newLog.contains(tokenSelesai)) {
                                        textview1.append(parseAnsi(newLog, true));
                                        textview1.invalidate();
	                                	textview1.requestLayout();
                                    }
                                });
            ...
            textview1.append(parseAnsi("[HM] ADB Error: " + e.getMessage() + "\n", true));
        }
    }
