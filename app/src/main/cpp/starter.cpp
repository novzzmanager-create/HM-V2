...
#define ARG_PUSH_DEBUG_ONLY(v, arg)
    ...
    snprintf(lib_path, PATH_MAX, "%s/lib/%s", dirname((char *) dex_path), ABI);

    ARG(argv)

    ARG_PUSH(argv, "/system/bin/app_process")
    ARG_PUSH_FMT(argv, "-Djava.class.path=%s", dex_path)
    ARG_PUSH_FMT(argv, "-Dhypers.library.path=%s", lib_path)
...
static void start_server(const char *path, const char *main_class, const char *process_name) {
    ...
    switch (pid) {
        ...
        default: {
            printf("info: hypers_server pid is %d\n", pid);
            printf("info: hypers_starter exit with 0\n");
            exit(EXIT_SUCCESS);
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    if (uid != 0 && uid != 2000) {
        perrorf("fatal: run Hypers from non root nor adb user (uid=%d).\n", uid);
        exit(EXIT_FATAL_UID);
    }
    ...
    if (uid == 0) {
        ...
        if (se::getcon(&context) == 0) {
            ...
            if (res != 0) {
                perrorf(
                    "fatal: the su you are using does not allow app "
                    "(u:r:untrusted_app:s0) to connect to su (%s) with binder.\n",
                    context
                );
    ...
    foreach_proc([](pid_t pid) {
        ...
        if (get_proc_name(pid, name, 1024) != 0)
            return;
        ...
        if (kill(pid, SIGKILL) == 0) {
            ...
            perrorf(
                "fatal: can't kill %d, please try to stop existing Hypers from app first.\n",
                pid
            );
