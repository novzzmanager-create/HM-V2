...
// Ambil SSL* dari SSLSocket via reflection, lalu panggil SSL_export_keying_material (BoringSSL)
static jbyteArray AdbPairingClient_ExportKeyingMaterial(JNIEnv *env, jclass clazz,
                                                          jobject jSSLSocket,
                                                          jstring jLabel,
                                                          jint length) {
    // Langkah 1: ambil SSLEngine dari SSLSocket
    jclass socketClass = env->GetObjectClass(jSSLSocket);
    jmethodID getEngineId = env->GetMethodID(socketClass, "getEngine", "()Ljavax/net/ssl/SSLEngine;");
    if (!getEngineId) {
        env->ExceptionClear();
        LOGE("getEngine not found");
        return nullptr;
    }
    jobject engine = env->CallObjectMethod(jSSLSocket, getEngineId);
    if (!engine) {
        LOGE("getEngine returned null");
        return nullptr;
    }

    // Langkah 2: ambil SSL pointer (long) dari engine Conscrypt
    jclass engineClass = env->GetObjectClass(engine);
    jmethodID getSslId = env->GetMethodID(engineClass, "SSL_to_long", "()J");
    if (!getSslId) {
        env->ExceptionClear();
        LOGE("SSL_to_long not found");
        return nullptr;
    }
    jlong sslPtr = env->CallLongMethod(engine, getSslId);
    SSL *ssl = reinterpret_cast<SSL *>(sslPtr);
    if (!ssl) {
        LOGE("SSL pointer is null");
        return nullptr;
    }

    // Langkah 3: panggil SSL_export_keying_material dari BoringSSL
    ...
    if (result != 1) {
        LOGE("SSL_export_keying_material failed");
        delete[] out;
        return nullptr;
    }

    jbyteArray jOut = env->NewByteArray(length);
    env->SetByteArrayRegion(jOut, 0, length, (jbyte *) out);
    delete[] out;
    return jOut;
}
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    env->RegisterNatives(
            env->FindClass("com/hypers/hm/adb/AdbPairingClient"),
            methods_AdbPairingClient,
    ...
    env->RegisterNatives(
            env->FindClass("com/hypers/hm/adb/AdbPairingClient$PairingContext"),
